...
impl Interpreter {
    ...
    fn register_standard_library(env: &Rc<RefCell<Environment>>) {
        ...
        env.register_native_fn("range", |_, args| {
            match args.as_slice() {
                ...
                [Value::Number(end)] => {
                    let range: Vec<Value> = (0..*end as i64)
                        .map(|i| Value::Number(i as f64))
                        .collect();
                    Ok(Value::List(range))
                },
        ...
        env.register_native_fn("invoke", |interpreter, args| {
            ...
            match function_value {
                Value::Function(func) => {
                    interpreter.call_function(func, remaining_args.to_vec())
                }
    ...
    fn evaluate(&self, node: &AstNode, env: &Rc<RefCell<Environment>>) -> Result<Value, RuntimeError> {
        match node {
            ...
            AstNode::Block(statements) => {
                ...
                for stmt in statements {
                    result = match self.evaluate(stmt, &block_env) {
                        Ok(val) => val,
                        Err(RuntimeError::ReturnValue(val)) => return Err(RuntimeError::ReturnValue(val)),
                        Err(e) => return Err(e),
                    };
                }
            ...
            AstNode::FunctionDef { name, params, body } => {
                ...
                env.borrow_mut().define(name, Value::Function(function.clone()));
            ...
            AstNode::FunctionCall { name, args } => {
                ...
                match function_value {
                    ...
                    Value::NativeFunction(native_fn) => {
                        (native_fn.function)(self, evaluated_args)
                    },
                    _ => Err(RuntimeError::TypeError(
            ...
            AstNode::IfStmt { condition, then_branch, elsif_branches, else_branch } => {
            ...
            AstNode::ForStmt { item, iterator, body } => {
            ...
            AstNode::SubscriptAssignment { name, indexes, value } => {
                let evaluated_value = self.evaluate(value, env)?;
    ...
    fn eval_binary_op(&self, op: &Operator, left: &Value, right: &Value) -> Result<Value, RuntimeError> {
        match (op, left, right) {
            // Numeric operations
            (Operator::Plus, Value::Number(a), Value::Number(b)) => {
                Ok(Value::Number(a + b))
            },
            (Operator::Minus, Value::Number(a), Value::Number(b)) => {
                Ok(Value::Number(a - b))
            },
            (Operator::Asterisk, Value::Number(a), Value::Number(b)) => {
                Ok(Value::Number(a * b))
            },
            ...
            // Numeric comparisons
            (Operator::Greater, Value::Number(a), Value::Number(b)) => {
                Ok(Value::Boolean(a > b))
            },
            (Operator::GreaterEqual, Value::Number(a), Value::Number(b)) => {
                Ok(Value::Boolean(a >= b))
            },
            (Operator::Less, Value::Number(a), Value::Number(b)) => {
                Ok(Value::Boolean(a < b))
            },
            (Operator::LessEqual, Value::Number(a), Value::Number(b)) => {
                Ok(Value::Boolean(a <= b))
            },
